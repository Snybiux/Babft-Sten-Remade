            ...
            local types = { } do
				...
				function types.input(inputOptions)
				    ...
				    local function setClipboardText(text)
				        pcall(function()
				            UserInputService:SetClipboard(tostring(text))
				        end)
				    end
				    ...
				    inner.MouseButton2Click:Connect(function()
				        if canType then
				    ...
				    UserInputService.InputBegan:Connect(function(inputObject)
				        ...
				        if canType and ctrlHeld then
				            -- Handle clipboard operations
				            if keycode == Enum.KeyCode.V then -- Paste
				                local clipboardText = getClipboardText()
				                textValue = textValue .. clipboardText
				                updateText()
				                self.event:Fire(textValue)
				                return
				            elseif keycode == Enum.KeyCode.C then -- Copy
				                setClipboardText(textValue)
				                return
				            elseif keycode == Enum.KeyCode.X then -- Cut
				                setClipboardText(textValue)
				                textValue = ""
				                updateText()
				                self.event:Fire(textValue)
				                return
				            elseif keycode == Enum.KeyCode.A then -- Select all
				                -- You might want to implement text selection visually
				                return
				            end
				        end
