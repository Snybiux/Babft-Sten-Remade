...
function Functions:Add(name, needed, missing)
    ...
    -- Bestimme die Hintergrundfarbe basierend auf fehlenden Blöcken
    local bgColor
    if missing and missing > 0 then
        -- Berechne den Prozentsatz der fehlenden Blöcke
        local missingPercentage = missing / needed
        
        if missingPercentage >= 1 then
            -- Rot (100% fehlend)
            bgColor = Color3.fromRGB(200, 50, 50)
        elseif missingPercentage >= 0.5 then
            -- Orange/Gelb (50-99% fehlend)
            bgColor = Color3.fromRGB(200, 150, 50)
        else
            -- Gelb (1-49% fehlend)
            bgColor = Color3.fromRGB(200, 200, 50)
        end
    else
        -- Grün (0% fehlend)
        bgColor = Color3.fromRGB(50, 200, 50)
    end
